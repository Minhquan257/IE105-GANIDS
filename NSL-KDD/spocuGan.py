...
def plot_kl(kl_history):
    ...
    fig.savefig('./imgs/nsl_relu_kl_div_plot.png',
...
def main(arg):
    ...
    to_drop = preprocessing.get_contant_featues(
        train, data_cols, threshold=0.995)
    ...
    x_train, x_test = preprocessing.preprocess(
        train, test, data_cols, "Robust", True)
    ...
    train_dataset = tf.data.Dataset.from_tensor_slices(
        (tf.convert_to_tensor(x), y)).batch(batch_size, drop_remainder=True)
    ...
    def train_step(X, y_label):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            disc_loss = discriminator_loss(
                cross_entropy, real_output, generated_output)
    ...
    for epoch in range(epochs):
        ...
        kl_div = calculate_kl_div(
            generator, noise, y.astype(np.float32), norm_p)
        kl_history.append(kl_div)
        g_losses.append(total_gen_loss / batch_size)
        disc_losses.append(total_disc_loss / batch_size)
        print('Time for epoch {:01.0f} is {:.5} sec - gen_loss = {:.5}, disc_loss = {:.5} kl_div = {:.5}'.format(
            epoch + 1, time.time() - start, total_gen_loss / batch_size, total_disc_loss / batch_size, kl_div))
    ...
    clf.compare(x, y.astype(np.float32), x_test[data_cols].values[for_test], y_test[for_test],
                generator, label_mapping, ml_classifiers, cv=5, rand_noise_dim=latent_dim)
